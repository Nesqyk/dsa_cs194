...
class Node:
    ...
    def __init__(self, data):
        ...
        self.next = None  # A pointer to the next node in the list
...
current_node = n1  # Start at the head of the list
print("\n--- Simple Standalone Traversal ---")
while current_node is not None:
    print(f"Found data: {current_node.data}")
    current_node = current_node.next # Move to the next node in the sequence

# Trace the Pointer: Explanation of loop termination
"""
The loop terminates when `current_node` becomes `None`. 
The `current_node = current_node.next` statement iterates through the list. When it reaches the last node (`n4`),
its `next` attribute is `None`. The next iteration of the loop's condition (`while current_node is not None`)
will evaluate to false, causing the loop to stop.
"""
# ----
...
class LinkedList:
    def __init__(self):
        self.head = None  # The head attribute manages the list's starting point

    # Activity 3.1: Refactor Traversal
    # This `display` method encapsulates the traversal logic within the class.
    def display(self):
        # The `self.head` attribute is used as the starting point for traversal.
    ...
    def delete_at_start(self):
        ...
        current = self.head # Save the old head for cleanup and message

        # 2. Pointer Focus: Analyze the step: self.head = self.head.next.
        # Explanation: This single line reassigns the list's `head` pointer to the second node in the list (the one `self.head.next` points to).
        # The original head is now only referenced by the `current` variable, effectively unlinking it from the list.
        self.head = self.head.next

        # 2. Clean-up: Why is current.next = None a good practice?
        # A good practice to explicitly break the link from the deleted node to the rest of the list. This prevents 'loitering' pointers and helps the garbage collector
        # reclaim the memory of the detached node cleanly, ensuring it isn't accidentally accessed later.
        current.next = None 
    ...
    def delete_by_value(self, value):
        ...
        if current and current.data == value:
            ...
            current.next = None # Cleanup
            print(f" -> Deleted '{value}' in the list (was the head)")
            return
        
        # Case 2: Finding the node to delete (and its predecessor)
        prev = None
        ...
        # Case 4: Unlinking the node (Middle or End)
        # 2. Critical Step: Identify the line in your code that performs the unlinking.
        # The unlinking line is:
        # prev.next = current.next 
        # Explain why this step is impossible without the prev pointer:
        # In a singly linked list, a node only knows about the *next* node. To remove 'current', you must modify the 'next' pointer of the node *before* it, which is 'prev'. 
        # Without 'prev', there is no way to reach back and change the pointer that points to 'current'.
        prev.next = current.next

        current.next = None # Cleanup
        # current = None # Optional: helps the garbage collector
        print(f" -> Deleted {value} in the list")

    # Activity 2.1: Implementing Search (search)
    def search(self, key):
        current = self.head
        # 2. Complexity: Why is this search operation O(n) while array access by index is O(1)?
        # Comment: O(n) for search in a Singly Linked List because we must start at the head and sequentially traverse (follow 'next' pointers) 
        # each node until the key is found or the end is reached. Array access is O(1) because elements are stored in contiguous memory, allowing 
        # direct calculation of an element's address using its index.
        while current: 
            if current.data == key:
    ...
    def access(self, index):
        ...
        while current is not None:
            ...
            # 5. Counter Logic: Analyze how the count variable is used. Why is traversing and using a counter the only way to find the i-th element in a standard Singly Linked List?
            # The count variable tracks the current position (index) in the list. Traversal and a counter are necessary because a Singly Linked List does not store elements in contiguous memory and lacks an index-based addressing scheme. 
            # We must sequentially follow the 'next' pointers from the head, incrementing the counter at each step, until the target index is reached.
            current = current.next
            count += 1
